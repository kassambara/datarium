...
# CLAUDE.html and gets listed in sitemap.xml. On a Bootstrap 5 template its full
# text is also baked into search.json (the site search index), which keeps the
# whole document searchable even after the .html is deleted. That happened here:
# docs/CLAUDE.html was rendered and committed with the 0.2.0 site rebuild, so it
# was served from GitHub until 2026-07-28. Note that docs/ is TRACKED in this
# repo -- a leaked page is public via GitHub as well as via the web host.
#
...
#      deployed.
#
# This script lives in .github/scripts/ on purpose: .Rbuildignore excludes
# .github/, and pkgdown does not copy it into the published site.
#
# Usage:
...
# leaves the entire document publicly searchable. Only Bootstrap 5 pkgdown
# templates emit it; this loop is a no-op until _pkgdown.yml moves to BS5.
...
echo "Deploy with a MIRRORING sync (rsync --delete) to nuxit:~/www/rpkgs/datarium. A plain"
echo "re-upload leaves an already-published CLAUDE.html and a stale search.json on the server."
